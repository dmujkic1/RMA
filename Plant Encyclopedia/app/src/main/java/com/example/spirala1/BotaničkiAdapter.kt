...
class BotaničkiAdapter(
    private val context: Context,
    private var biljke: List<Biljka>,
    private val onItemClicked: (biljka:Biljka) -> Unit) : RecyclerView.Adapter<BotaničkiAdapter.BiljkaViewHolder>(){

    private var trefle=TrefleDAO()
    private var uradjenaPretraga = false
    ...
    override fun onBindViewHolder(holder: BiljkaViewHolder, position: Int) {
        ...
        if (!biljka.klimatskiTipovi.isNullOrEmpty()){ holder.klima.text = biljka.klimatskiTipovi[0].opis }
        else{ holder.klima.text="" }

        if (!biljka.zemljisniTipovi.isNullOrEmpty()) { holder.zemlja.text = biljka.zemljisniTipovi[0].naziv }
        else{ holder.zemlja.text="" }
    ...
    fun postaviNaTruePretragu(){
        this.uradjenaPretraga=true
    }

    fun getPretragaBool(): Boolean{
        return this.uradjenaPretraga
    }

    fun postaviNaFalsePretragu(){
